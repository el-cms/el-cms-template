...
private function _nsfw() {
		if (isset($this->request->params['named']['switchNSFW'])) {
			if ($this->request->params['named']['switchNSFW'] == 'hide') {
				$this->Session->setFlash(__('Not safe for work content will be hidden'), 'flash_success');
				$this->Session->write('see_nsfw', false);
				return false;
			} else {
				$this->Session->setFlash(__('Not safe for work content will be shown'), 'flash_success');
				$this->Session->write('see_nsfw', true);
				return true;
			}
		}
		return $this->Session->read('see_nsfw');
	}
<?php
endif;
